...
export const convertAnalysisToInitialData = (results: AnalysisResult): AppApproachData => {
    ...
    for (const airport of Object.values(results.airports)) {
        for (const approach of airport.approaches) {
            approaches.push({
                ...
                has_hold_in_lieu_of_procedure_turn: approach.has_hold_in_lieu_of_procedure_turn,
                has_dme_arc: approach.has_dme_arc,
            })
        }

        airports[airport.id] = { name: airport.name, location: convertLatitudeLongitude(airport) };
    }
 ...
 *   to [32.41, -99.68]
 */
const convertLatitudeLongitude = (location: { latitude: string, longitude: string }): LatLngType => {
    return {
